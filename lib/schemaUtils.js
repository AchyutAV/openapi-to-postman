  ...
  * @param {object} schemaCache - object storing schemaFaker and schmeResolution caches
  ...
  convertToPmBody: function(requestBody, requestType, components, options, schemaCache) {
    ...
    if (contentObj.hasOwnProperty(URLENCODED)) {
      ...
      else if (contentObj.hasOwnProperty(APP_JSON)) { bodyType = allVal; }
      else if (contentObj.hasOwnProperty(TEXT_HTML)) { bodyType = TEXT_HTML; }
      else if (contentObj.hasOwnProperty(TEXT_PLAIN)) { bodyType = allVal; }
  ...
  * @param {object} schemaCache - object storing schemaFaker and schmeResolution caches
  ...
  convertToPmResponse: function(response, code, originalRequest, components, options, schemaCache) {
    ...
    if (responseBodyWrapper.contentTypeHeader) {
      ...
      responseHeaders.push({ key: 'Content-Type', value: 'application/json' });
  ...
  sanitizeUrlPathParams: function (reqUrl, pathVars) {
    ...
    if (matches) {
      matches.forEach((match) => {
        const replaceWith = match.replace(/{{[a-zA-z]*}}|\\/g, this.makeid(7)).replace(/}}/g, '');
  ...
  */
  convertRequestToItem: function(openapi, operationItem, components, options, schemaCache, variableStore) {
    ...
    var reqName,
      ...
      displayforEx;
      exUrl = openapi.servers[1].variables.protocol.default+'://'+openapi.servers[1].variables.server.default+':'+openapi.servers[1].variables.port.default+openapi.baseUrlVariables.basePath.default
    ...
    // adding example path variables inside the postman
    exReqUrl = this.fixPathVariablesInUrl(exReqUrl);
    ...
    // updating example request variables
    exReqUrl = sanitizeResult.url;
    ...
    if (operation.responses) {
      ...
      _.forOwn(operation.responses, (response, code) => {
        ...
        //settting URL for example variables
        thisOriginalRequest.url = displayforEx;
